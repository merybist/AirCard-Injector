...
elif [ -d "$ROOT/helpers/idevice_pair.app" ]; then
    echo "==> Embedding bundled idevice_pair.app helper..."
    cp -R "$ROOT/helpers/idevice_pair.app" "$BUILD_DIR/AirCardInjector.app/Contents/Helpers/"
    codesign --force --deep --sign - "$BUILD_DIR/AirCardInjector.app/Contents/Helpers/idevice_pair.app"
fi
